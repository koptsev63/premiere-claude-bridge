...
class ResolveAdapter(NLEAdapter):
    ...
    def apply_corrections(self, corrections: dict) -> dict:
        """Apply per-clip cleanup on the current timeline.

        `corrections` is {clip_path: core.cleanup.Correction}.

        - Horizon: undo tilt via TimelineItem.SetProperty("RotationAngle").
        - Stabilization: call TimelineItem.Stabilize() — Resolve's OWN
          professional stabilizer (Similarity/Perspective motion model),
          not ffmpeg `deshake`. This is the quality path and it runs
          inside DaVinci. Verified against the shipped scripting API
          (README: `Stabilize() --> Bool`).

        Returns {clip_name: {"rotate": deg?, "stabilized": bool?}}.
        Call after apply_cutlist().
        """
        tl = self._project.GetCurrentTimeline()
        if tl is None:
        ...
        for item in tl.GetItemListInTrack("video", 1) or []:
            ...
            if nm in rot:
                try:
                    item.SetProperty("RotationAngle", rot[nm])
                    rec["rotate"] = rot[nm]
                except Exception as e:  # noqa: BLE001
                    rec["rotate"] = f"FAILED:{e!r}"
            if nm in stab:
                try:
                    rec["stabilized"] = bool(item.Stabilize())
                except Exception as e:  # noqa: BLE001
                    rec["stabilized"] = f"FAILED:{e!r}"
            if rec:
                applied[nm] = rec
        return applied
